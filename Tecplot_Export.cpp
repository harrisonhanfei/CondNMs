...
/* int Tecplot_Export::Export_network_threads(const Rectangle &cub, const vector<vector<Point_2D> > &cnts_points)const
{
	...
	if(Export_rectangle(otec, cub)==0) return 0;
...
int Tecplot_Export::Export_reactangle(ofstream &otec, const Rectangle &cell)const
{
	otec << "ZONE N=" << 8 << ", E=" << 1 << ", F=FEPOINT, ET=BRICK" << endl;
	double cell_x[2] = {cell.point[0].x, cell.point[0].x+cell.len_x};
	double cell_y[2] = {cell.point[0].y, cell.point[0].y+cell.wid_y};
	for(int i=0; i<2; i++)
		for(int j=0; j<2; j++)
			{
				otec << cell_x[i] << "  " << cell_y[j] << endl;
			}
...
//---------------------------------------------------------------------------
//Export 2D nanowire threads
int Tecplot_Export::Export_nano_threads(ofstream &otec, const vector<vector<Point_2D> > &cnts_points)const
{
...
int Tecplot_Export::Export_cnt_network_meshes(const struct cuboid &cub, const vector<vector<Point_2D> > &cnts_points, const vector<double> &cnts_radius)const
{
...
int Tecplot_Export::Export_cnt_network_meshes(const struct cuboid &cub, const vector<Point_2D> &cnts_points, const vector<double> &cnts_radius, const vector<vector<long int> > &structure, string filename)const
{
    //For storing the nodes and elements to construct nanowires with diameters
    vector<vector<Node> > cnts_nodes;
    vector<vector<Element> > cnts_eles;
...
//---------------------------------------------------------------------------
//Export nanowire network by tetrahedron elements (Multiple zones in tecplot: each nanowire by one zone) with a specific filename
...
//---------------------------------------------------------------------------
//Export nanowire network by tetrahedron elements (Single zones in tecplot: all nanowires by one zone) with a specific filename
