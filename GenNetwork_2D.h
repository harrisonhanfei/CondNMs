...
public:
	...
	//int Generate_cnts_nodes_elements(vector<vector<Node> > &nodes, vector<vector<Element> > &eles, const vector<vector<Point_2D> > &cnts_points, const vector<double> &cnts_radius);
	//      //Generate the nodes and tetrahedron elements of nanowires (No const following this function because a sum operation on two Point_2D points inside). This function uses a 1D point vector and a 2D structure vector that references the point vector
	//      int Generate_cnts_nodes_elements(vector<vector<Node> > &nodes, vector<vector<Element> > &eles, const vector<Point_2D> &cnts_points, const vector<double> &cnts_radius, const vector<vector<long int> > &structure);
